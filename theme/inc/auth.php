...
add_action( 'wp_ajax_aurora_verify_password', 'aurora_verify_password_ajax' );
function aurora_verify_password_ajax() {
    aurora_verify_ajax_nonce();
    ...
    if ( ! $password ) {
        wp_send_json_error( [ 'message' => __( 'Please enter your password.', 'aurora' ) ] );
    }

    if ( ! wp_check_password( $password, $user->user_pass ) ) {
        wp_send_json_error( [ 'message' => __( 'Password is incorrect.', 'aurora' ) ] );
    }

    wp_send_json_success( [ 'message' => __( 'Password verified successfully.', 'aurora' ) ] );
}

// === Account: Change Password (After Verification) ===
add_action( 'wp_ajax_aurora_change_password_verified', 'aurora_change_password_verified_ajax' );
function aurora_change_password_verified_ajax() {
    aurora_verify_ajax_nonce();
    ...
    wp_send_json_success( [ 'message' => __( 'Password changed successfully.', 'aurora' ) ] );
}

// === Account: Send Password Reset OTP ===
add_action( 'wp_ajax_aurora_send_password_reset_otp', 'aurora_send_password_reset_otp_ajax' );
function aurora_send_password_reset_otp_ajax() {
    aurora_verify_ajax_nonce();
    ...
    if ( ! $email || ! is_email( $email ) ) {
        wp_send_json_error( [ 'message' => __( 'Invalid email address.', 'aurora' ) ] );
    }

    if ( $email !== $user->user_email ) {
        wp_send_json_error( [ 'message' => __( 'Email does not match your account.', 'aurora' ) ] );
    }

    // Issue OTP for password reset
    $result = aurora_issue_otp( $user->ID, $user->user_email, 'password_reset' );
...
add_action( 'wp_ajax_aurora_reset_password_with_otp', 'aurora_reset_password_with_otp_ajax' );
function aurora_reset_password_with_otp_ajax() {
    aurora_verify_ajax_nonce();
    ...
    if ( ! $otp_code ) {
        wp_send_json_error( [ 'message' => __( 'Please enter OTP.', 'aurora' ) ] );
    }

    // Verify OTP
    $verify = aurora_verify_otp_code( $user->ID, $user->user_email, $otp_code, 'password_reset' );
    if ( ! $verify['success'] ) {
        wp_send_json_error( $verify );
    }
