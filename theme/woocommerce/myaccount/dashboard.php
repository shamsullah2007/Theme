        ...
        <div class="settings-section">
            <div class="settings-card">
                ...
                <div id="password-tab" class="tab-content" style="display: none;">
                    <div class="setting-item">
                        <label><?php esc_html_e( 'Password Status', 'aurora' ); ?></label>
                        <input type="text" class="form-input" value="<?php esc_html_e( 'Password is set', 'aurora' ); ?>" disabled>
                    </div>

                    <button class="button button-primary" id="change-password-btn" style="display: block; margin-top: 15px;"><?php esc_html_e( 'Change Password', 'aurora' ); ?></button>
                    ...
                    <div id="password-change-form" style="display: none; margin-top: 20px;">
                        ...
                        <div id="current-password-section">
                            ...
                            <button class="button button-primary" id="verify-current-password"><?php esc_html_e( 'Verify Password', 'aurora' ); ?></button>
                            <button class="button" id="cancel-password-change"><?php esc_html_e( 'Cancel', 'aurora' ); ?></button>
                        </div>

                        <!-- Step 2: Enter New Password (shown after verification) -->
                        <div id="new-password-section" style="display: none; margin-top: 15px;">
                            ...
                            <div class="setting-item">
                                <label><?php esc_html_e( 'Confirm Password', 'aurora' ); ?></label>
                                <input type="password" id="confirm-password" class="form-input" placeholder="<?php esc_attr_e( 'Confirm your new password', 'aurora' ); ?>">
                            </div>

                            <button class="button button-primary" id="submit-new-password"><?php esc_html_e( 'Change Password', 'aurora' ); ?></button>
                        </div>

                        <!-- Forgot Password Option -->
                        <div id="forgot-password-option" style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                            <p style="margin-bottom: 10px;"><?php esc_html_e( 'Forgot your password?', 'aurora' ); ?></p>
                            <button class="button button-secondary" id="use-otp-reset"><?php esc_html_e( 'Use OTP Instead', 'aurora' ); ?></button>
                        </div>
                    </div>

                    <!-- OTP Reset Password Form -->
                    <div id="otp-password-reset-form" style="display: none; margin-top: 20px;">
                        <div class="setting-item">
                            <label><?php esc_html_e( 'Email Address', 'aurora' ); ?></label>
                            <input type="email" id="reset-email" class="form-input" value="<?php echo esc_attr( wp_get_current_user()->user_email ); ?>" disabled>
                        </div>

                        <button class="button button-primary" id="send-password-reset-otp"><?php esc_html_e( 'Send OTP', 'aurora' ); ?></button>
                        <button class="button" id="cancel-otp-reset"><?php esc_html_e( 'Cancel', 'aurora' ); ?></button>

                        <!-- OTP Verification Section -->
                        <div id="otp-verification-section" style="display: none; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                            <div class="setting-item">
                                <label><?php esc_html_e( 'Enter OTP (sent to your email)', 'aurora' ); ?></label>
                                <input type="text" id="password-reset-otp-code" class="form-input" placeholder="<?php esc_attr_e( '000000', 'aurora' ); ?>" maxlength="6">
                            </div>

                            <div class="setting-item">
                                <label><?php esc_html_e( 'New Password', 'aurora' ); ?></label>
                                <input type="password" id="otp-new-password" class="form-input" placeholder="<?php esc_attr_e( 'Enter new password', 'aurora' ); ?>">
                                <small><?php esc_html_e( 'Password must be at least 8 characters long', 'aurora' ); ?></small>
                            </div>

                            <div class="setting-item">
                                <label><?php esc_html_e( 'Confirm Password', 'aurora' ); ?></label>
                                <input type="password" id="otp-confirm-password" class="form-input" placeholder="<?php esc_attr_e( 'Confirm your new password', 'aurora' ); ?>">
                            </div>

                            <button class="button button-primary" id="confirm-otp-password-reset"><?php esc_html_e( 'Change Password', 'aurora' ); ?></button>
                        </div>
                    </div>
                </div>
                </div>
...
jQuery(document).ready(function($) {
    ...
    $('#change-password-btn').on('click', function() {
        ...
        $('#forgot-password-option').show();
        $('#otp-password-reset-form').hide();
    ...
    $('#verify-current-password').on('click', function() {
        const currentPassword = $('#current-password').val();

        if (!currentPassword) {
            showMessage('<?php esc_html_e( 'Please enter your current password', 'aurora' ); ?>', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $('#current-password-section').hide();
                    $('#new-password-section').show();
                    $('#forgot-password-option').hide();
                    $('#new-password').focus();
                } else {
                    showMessage(response.data.message, 'error');
                    passwordVerified = false;
            ...
            error: function() {
                showMessage('<?php esc_html_e( 'Error verifying password', 'aurora' ); ?>', 'error');
            }
        });
    });
    ...
    $('#submit-new-password').on('click', function() {
        ...
        if (!newPassword || newPassword.length < 8) {
            showMessage('<?php esc_html_e( 'Password must be at least 8 characters long', 'aurora' ); ?>', 'error');
            return;
        }

        if (newPassword !== confirmPassword) {
            showMessage('<?php esc_html_e( 'Passwords do not match', 'aurora' ); ?>', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'aurora_change_password_verified',
                nonce: nonce,
            ...
            error: function() {
                showMessage('<?php esc_html_e( 'Error changing password', 'aurora' ); ?>', 'error');
            }
        });
    });

    // Switch to OTP reset
    $('#use-otp-reset').on('click', function() {
        $('#current-password-section').hide();
        $('#new-password-section').hide();
        $('#forgot-password-option').hide();
        $('#password-change-form').hide();
        $('#otp-password-reset-form').show();
        passwordVerified = false;
    });

    // Cancel OTP reset and go back
    $('#cancel-otp-reset').on('click', function() {
        $('#otp-password-reset-form').hide();
        $('#password-change-form').show();
        $('#current-password-section').show();
        $('#new-password-section').hide();
        $('#forgot-password-option').show();
        $('#current-password').val('');
        $('#password-reset-otp-code').val('');
        $('#otp-new-password').val('');
        $('#otp-confirm-password').val('');
    });

    // Send OTP for password reset
    $('#send-password-reset-otp').on('click', function() {
        const email = $('#reset-email').val();

        if (!email) {
            showMessage('<?php esc_html_e( 'Email is required', 'aurora' ); ?>', 'error');
            return;
        }
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'aurora_send_password_reset_otp',
                nonce: nonce,
                email: email
            },
            success: function(response) {
                if (response.success) {
                    ...
                    $('#send-password-reset-otp').prop('disabled', true);
                    $('#password-reset-otp-code').focus();
                } else {
                    showMessage(response.data.message, 'error');
                }
            },
            error: function() {
                showMessage('<?php esc_html_e( 'Error sending OTP', 'aurora' ); ?>', 'error');
            }
        });
    });

    // Confirm password reset with OTP
    $('#confirm-otp-password-reset').on('click', function() {
        const otpCode = $('#password-reset-otp-code').val();
        const newPassword = $('#otp-new-password').val();
        const confirmPassword = $('#otp-confirm-password').val();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'aurora_reset_password_with_otp',
                nonce: nonce,
                otp: otpCode,
                new_password: newPassword
            },
            ...
            error: function() {
                showMessage('<?php esc_html_e( 'Error resetting password', 'aurora' ); ?>', 'error');
            }
        });
    });
